...
function setup() {
 ...
 monsterGroup=new Group()
  ...
  gate=createSprite(360,335,100,20)
...
function draw() {
 ...
 textSize (30)
 fill ("white")
 text("LIFE="+ life,700,100)
  text (mouseX+";"+mouseY,100,200)

  if(pacman1.body.isTouching(monsterGroup)&& life!==0){
    pacman1.body.x=350
    pacman1.body.y=620
    life--
  }
  if(life===0){
    gameState="END"
    restartButton=createSprite(265,410,20,20)
    restartButton.addImage(restartImg)
    restartButton.scale=0.2
  
  }
  if(gameState==="END"){
    pacman1.body.visible=false
    textSize(50)
    fill("white")
    text("PAC-MAN IS NO MORE",200,237) 
    if(mousePressedOver(restartButton))
     gameState="PLAY" 
     pacman1.body.visible=true
  ...
  if(gameState==="PLAY"){
    ...
    if(keyDown("right")){
      ...
      if(keyDown("down")){
            ...
            pacman1.body.collide(keyGroup,keyKill)
             pacman1.body.collide(gate)
             if(score===5){
               gate.destroy()
           ...
           if (pacman1.body.isTouching(trophy)){
             textSize(30)
             fill("white")
